...
iterator_target = iter(val_dl_target)
# kernel = np.ones((5, 5))
take_classes = [3, 4, 5, 6, 7, 9, 11, 12, 14, 15, 16, 17, 18]

for num_image, (image, label, image_path, label_path) in enumerate(tqdm(val_dl_target2)):
    ...
    if "gtFine" in label_path[0]:
        ...
        with torch.no_grad():
            logits = model(image, size=tuple(cfg.dataset.input_size_val))
            # flipped = model(tf.hflip(image), size=tuple(cfg.dataset.input_size_val))
            # flipped = tf.hflip(flipped)
            # logits_scaled_up= model(scaled_image, size=tuple(cfg.dataset.input_size_val))
    ...
    for ii in range(1):
        ...
        with torch.no_grad():
            logits = model(image_target, size=tuple(cfg.dataset.input_size_val))
            # flipped = model(tf.hflip(image_target), size=tuple(cfg.dataset.input_size_val))
            # flipped = tf.hflip(flipped)
            # logits_scaled_up= model(scaled_image, size=tuple(cfg.dataset.input_size_val))
    ...
    if "gtFine" in label_path[0]:
        dest_path_img = image_path[0].replace("left", "images_distill_52_no_erosion", 1)
        dest_path_label = label_path[0].replace("gtFine", "semantic_distill_52_no_erosion", 1)
    else:
        dest_path_img = image_path[0].replace("images_translated_bdl", "images_distill_52_no_erosion", 1)
        dest_path_label = label_path[0].replace("semantic_encoded", "semantic_distill_52_no_erosion", 1)

    # dest_path_img = "/data/aCardace/datasets/GTA5/images_baseline_no_IT/" + str(num_image) + ".png"
    # dest_path_label = "/data/aCardace/datasets/GTA5/baseline_no_IT_semantic_encoded/" + str(num_image) + ".png"
    ...
    augmented_source_image = val_ds_target.re_normalize(augmented_source_image.cpu().numpy().squeeze(), gbr=False)
