...
model.to(device)
model.eval()
_, _, val_transform = get_transforms(cfg)
...
val_ds_source = SegmentationDataset(
    ...
    transform=val_transform,
    encode=cfg.dataset.encode,
    encoding=cfg.dataset.source_encoding,                            #CHECK!!!!!!
    num_classes=cfg.dataset.num_classes
)
...
iterator_target = iter(val_dl_target)
kernel = np.ones((5, 5))
take_classes = [3, 4, 5, 6, 7, 10, 11, 12, 13, 14, 15]

for num_image, (image, label, image_path, label_path) in enumerate(tqdm(val_dl_source)):
    label = tf.resize(label, tuple(cfg.dataset.input_size_val_target), interpolation=PIL.Image.NEAREST)
    ...
    if "gtFine" in label_path[0]:
        ...
        with torch.no_grad():
            logits= model(image, size=tuple(cfg.dataset.input_size_val_target))
            flipped= model(tf.hflip(image), size=tuple(cfg.dataset.input_size_val_target))
            flipped = tf.hflip(flipped)
            logits_scaled_up= model(scaled_image, size=tuple(cfg.dataset.input_size_val_target))
    ...
    for ii in range(1):
        ...
        with torch.no_grad():
            logits= model(image_target, size=tuple(cfg.dataset.input_size_val_target))
            flipped= model(tf.hflip(image_target), size=tuple(cfg.dataset.input_size_val_target))
            flipped = tf.hflip(flipped)
            logits_scaled_up= model(scaled_image, size=tuple(cfg.dataset.input_size_val_target))
        ...
        new_mask = torch.ones_like(predicted_label) * 16
        rand_classes = take_classes
    ...
    # if "gtFine" in label_path[0]:
    #     dest_path_img = image_path[0].replace("left", "w_images_baseline_synthia_ablation", 1)
    #     dest_path_label = label_path[0].replace("gtFine", "w_semantic_encoded_baseline_synthia_ablation", 1)
    # else:
    ...
    dest_path_img = "/data/aCardace/datasets/SYNTHIA_RAND_CITYSCAPES/images_baseline_proda_no_aug_fix_bn_69/" + str(num_image) + ".png"
    dest_path_label = "/data/aCardace/datasets/SYNTHIA_RAND_CITYSCAPES/semantic_baseline_proda_no_aug_fix_bn_69/" + str(num_image) + ".png"
